(ns clumcl.uts.security
  ...
  (:require [taoensso.timbre :as timbre
             :refer (trace debug info warn error fatal spy with-log-level)]) (:import [gov.nih.nlm.uts.webservice.security UtsWsSecurityControllerImplService UtsFault_Exception]))
...
(defn- ticket-granting-ticket
  "Gets a proxy grant ticket that is valid for 8 hours. Returns a 2-vector with the ticket as the first element and a timestamp for the second."
  [{:keys [umls-user umls-password]}]
  (try
    (info "Generating proxy grant ticket for user" umls-user)
    [(.getProxyGrantTicket security-service umls-user umls-password) (System/currentTimeMillis)]
...
(defn single-use-ticket
  "Get a single use ticket from the security web service. It will automatically renew the proxy granting ticket if it has expired."
  [conn]
  (let [tgt-map (get @connections conn)]
    (if (nil? tgt-map)
      (throw (IllegalArgumentException. "No connection for connection id. Was connection closed already?"))
...
(defn connect
  "Sets up a connection to the service using the supplied credentials. Credentials should be a map of the form:
